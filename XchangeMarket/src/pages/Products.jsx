...
import { FaSpinner, FaBoxOpen, FaSearch, FaFilter, FaMapMarkerAlt, FaLocationArrow, FaChevronDown, FaTimesCircle } from 'react-icons/fa';
...
const Products = () => {
    ...
    // Location state from custom hook
    const {
        selectedCity,
        filterMode,
        cities,
    ...
    const fetchProducts = useCallback(async () => {
        try {
            ...
            if (filterMode === 'city') {
                response = await productAPI.getProductsByLocation({
    ...
    const emptyStateMessage = useMemo(() => {
        ...
        if (filterMode === 'city') {
            return {
                title: `No products found in ${selectedCity}`,
                sub: `There are currently no listings in ${selectedCity}. Try a different city or browse all products.`,
            };
        }
        return {
    ...
    }, [searchQuery, filterMode, selectedCity, selectedCategory]);
            ...
            <div className="container mx-auto px-4 max-w-7xl">
                ...
                <div className="mb-10 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-6 relative z-50">
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-center justify-between pb-2 relative z-50">
                        ...
                        {/* 2. Filter Box Dropdown */}
                        <div id="location-dropdown-container" className="relative w-full md:w-1/4 text-left flex-shrink-0">
                            <button
                                onClick={() => setShowLocationDropdown(!showLocationDropdown)}
                                className={`w-full flex items-center justify-between bg-gray-50 border ${filterMode !== 'all' ? 'border-red-300 bg-red-50' : 'border-gray-200'} text-gray-900 text-sm rounded-xl px-4 py-3 hover:bg-gray-100 transition-colors`}
                            >
                                <div className="flex items-center gap-2">
                                    <FaMapMarkerAlt className={`flex-shrink-0 text-red-500`} />
                                    <span className="font-medium truncate font-bold text-sm">
                                        {filterMode === 'all' ? 'All Districts' : locationLabel}
                                    </span>
                                </div>
                                <div className="flex items-center gap-2">
                                    {filterMode !== 'all' && (
                                        <button
                                            ...
                                            className="hover:text-red-900 transition-colors text-gray-500"
                                            title="Clear location filter"
                                        >
                                            <FaTimesCircle className="text-sm text-red-500 hover:text-red-700" />
                                        </button>
                                    )}
                                    <FaChevronDown className={`text-gray-400 text-xs flex-shrink-0 transition-transform ${showLocationDropdown ? 'rotate-180' : ''}`} />
                                </div>
                            </button>
                            ...
                            <AnimatePresence>
                                {showLocationDropdown && (
                                    <motion.div
                                        ...
                                        exit={{ opacity: 0, y: 10 }}
                                        className="absolute z-50 top-full left-0 mt-2 w-full max-h-72 overflow-y-auto bg-white border border-gray-100 rounded-xl shadow-xl custom-scrollbar"
                                    >
                                        <button
                                            onClick={() => {
                                                clearLocation();
                                            ...
                                            className={`w-full text-left px-4 py-3 text-sm hover:bg-gray-50 transition-colors flex items-center gap-2 font-bold ${filterMode === 'all' ? 'text-red-600 bg-red-50/50' : 'text-gray-700'}`}
                                        >
                                        ...
                                        {cities.filter(c => c !== 'All Cities').map((city) => (
                                            <button
                                                ...
                                                className={`w-full text-left px-4 py-3 text-sm hover:bg-gray-50 transition-colors flex items-center gap-2
                                                    ${selectedCity === city && filterMode === 'city' ? 'font-bold text-red-600 bg-red-50/50' : 'text-gray-700'}
                                                `}
                                            >
                ...
                {loading ? (
                    <div className="flex flex-col justify-center items-center h-64">
                        ...
                        <p className="text-gray-500 font-medium animate-pulse">
                            {filterMode === 'city'
                                ? `Loading products in ${selectedCity}...`
                                : 'Loading products...'}
                        </p>
                    </div>
                    ...
                    >
                        <FaMapMarkerAlt className={`text-7xl mb-4 ${filterMode !== 'all' ? 'text-red-200' : 'text-gray-200'}`} />
                                ...
                                if (product.images) {
                                    if (Array.isArray(product.images)) {
                                        ...
                                        try {
                                            ...
                                            safeImages = product.images.trim().startsWith('[') ? [] : [product.images];
                                        }
                                    }
                                }
