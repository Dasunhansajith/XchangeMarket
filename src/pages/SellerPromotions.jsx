...
const SellerPromotions = () => {
    ...
    const loadPromotions = async () => {
        setLoading(true);
        try {
            // Uses the /promotions/seller endpoint which is scoped server-side
            // to the authenticated user — only returns THIS seller's promotions
            const myPromos = await promotionService.getSellerPromotions();
    ...
    const handleCreate = async (e) => {
        ...
        try {
            await promotionService.createPromotion({
                ...formData,
                sellerId: user?.email // Scoping to current seller
            });
        ...
        <div className="space-y-8 p-6 bg-white rounded-2xl shadow-sm border border-gray-100">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Boost your sales with limited-time offers and coupons.</p>
                </div>
                <button 
                ...
                >
                    {isAdding ? 'Cancel' : <><FaPlus /> Create Promotion</>}
                </button>
            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {promotions.length === 0 ? (
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-black text-gray-800 tracking-tight">No campaigns active</h3>
                            <p className="text-gray-400 mt-2 font-medium">Your creative discounts will appear here.</p>
                        </motion.div>
                    ) : (
